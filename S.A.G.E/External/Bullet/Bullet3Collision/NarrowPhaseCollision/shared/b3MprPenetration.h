...
inline int b3MprEq(float _a, float _b)
{
	...
	ab = static_cast<float>(B3_MPR_FABS(_a - _b));
	if (B3_MPR_FABS(ab) < FLT_EPSILON)
		return 1;

	a = static_cast<float>(B3_MPR_FABS(_a));
	b = static_cast<float>(B3_MPR_FABS(_b));
	if (b > a)
	{
...
B3_STATIC void b3FindPos(const b3MprSimplex_t *portal, b3Float4 *pos)
{
	...
	for (i = 0; i < 4; i++)
	{
		b3MprVec3Copy(&vec, &b3MprSimplexPoint(portal, static_cast<int>(i))->v1);
		b3MprVec3Scale(&vec, b[i]);
		b3MprVec3Add(&p1, &vec);

		b3MprVec3Copy(&vec, &b3MprSimplexPoint(portal, static_cast<int>(i))->v2);
		b3MprVec3Scale(&vec, b[i]);
		b3MprVec3Add(&p2, &vec);
	}
