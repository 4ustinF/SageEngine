...
public:
	...
	virtual void updateId()
	{
		...
		for (int i = 0; i < m_softBodies.size(); ++i)
		{
			...
			{
				psb->m_nodes[j].index = static_cast<int>(node_id);
				m_nodes.push_back(&psb->m_nodes[j]);
				++node_id;
			}
			for (int j = 0; j < psb->m_faces.size(); ++j)
			{
				psb->m_faces[j].m_index = static_cast<int>(face_id);
				++face_id;
			}
		}
	}
	...
	void calculateContactForce(const TVStack& dv, const TVStack& rhs, TVStack& f)
	{
		...
		for (int i = 0; i < m_softBodies.size(); ++i)
		{
			...
			for (int j = 0; j < psb->m_nodes.size(); ++j)
			{
				...
				f[static_cast<int>(counter)] = (node.m_im == 0) ? btVector3(0, 0, 0) : dv[static_cast<int>(counter)] / node.m_im;
				++counter;
			}
		}
		...
		{
			f[static_cast<int>(counter)] = rhs[static_cast<int>(counter)] - f[static_cast<int>(counter)];
		}
	}
};
