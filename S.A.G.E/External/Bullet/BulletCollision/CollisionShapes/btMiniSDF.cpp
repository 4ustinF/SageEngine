...
bool btMiniSDF::load(const char* data, int size)
{
	...
	btSdfDataStream ds(data, size);
	{
		double buf[6];
		ds.read(buf);
		m_domain.m_min[0] = static_cast<btScalar>(buf[0]);
		m_domain.m_min[1] = static_cast<btScalar>(buf[1]);
		m_domain.m_min[2] = static_cast<btScalar>(buf[2]);
		m_domain.m_min[3] = 0;
		m_domain.m_max[0] = static_cast<btScalar>(buf[3]);
		m_domain.m_max[1] = static_cast<btScalar>(buf[4]);
		m_domain.m_max[2] = static_cast<btScalar>(buf[5]);
		m_domain.m_max[3] = 0;
	}
	{
		unsigned int buf2[3];
	...
	}
	{
		double buf[3];
		ds.read(buf);
		m_cell_size[0] = static_cast<btScalar>(buf[0]);
		m_cell_size[1] = static_cast<btScalar>(buf[1]);
		m_cell_size[2] = static_cast<btScalar>(buf[2]);
	}
	{
		double buf[3];
		ds.read(buf);
		m_inv_cell_size[0] = static_cast<btScalar>(buf[0]);
		m_inv_cell_size[1] = static_cast<btScalar>(buf[1]);
		m_inv_cell_size[2] = static_cast<btScalar>(buf[2]);
	}
	{
...

	{
		...
		m_cell_map.resize(static_cast<int>(n_cell_maps0));
		for (int i = 0; i < n_cell_maps0; i++)
		{
			...
			cell_maps.resize(static_cast<int>(n_cell_maps1));
...
btMiniSDF::subdomain(btMultiIndex const& ijk) const
{
	btAssert(m_isValid);
	btVector3 tmp;
	tmp.m_floats[0] = static_cast<btScalar>(m_cell_size[0] * (double)ijk.ijk[0]);
	tmp.m_floats[1] = static_cast<btScalar>(m_cell_size[1] * (double)ijk.ijk[1]);
	tmp.m_floats[2] = static_cast<btScalar>(m_cell_size[2] * (double)ijk.ijk[2]);
...
btMiniSDF::shape_function_(btVector3 const& xi, btShapeGradients* gradient) const
{
	...
	btScalar _1mx = static_cast<btScalar>(1.0 - x);
	btScalar _1my = static_cast<btScalar>(1.0 - y);
	btScalar _1mz = static_cast<btScalar>(1.0 - z);

	btScalar _1px = static_cast<btScalar>(1.0 + x);
	btScalar _1py = static_cast<btScalar>(1.0 + y);
	btScalar _1pz = static_cast<btScalar>(1.0 + z);

	btScalar _1m3x = static_cast<btScalar>(1.0 - 3.0 * x);
	btScalar _1m3y = static_cast<btScalar>(1.0 - 3.0 * y);
	btScalar _1m3z = static_cast<btScalar>(1.0 - 3.0 * z);

	btScalar _1p3x = static_cast<btScalar>(1.0 + 3.0 * x);
	btScalar _1p3y = static_cast<btScalar>(1.0 + 3.0 * y);
	btScalar _1p3z = static_cast<btScalar>(1.0 + 3.0 * z);
	...
	btScalar _1mx2 = static_cast<btScalar>(1.0 - x2);
	btScalar _1my2 = static_cast<btScalar>(1.0 - y2);
	btScalar _1mz2 = static_cast<btScalar>(1.0 - z2);

	// Corner nodes.
	btScalar fac = static_cast<btScalar>(1.0 / 64.0 * (9.0 * (x2 + y2 + z2) - 19.0));
	...
	fac = static_cast<btScalar>(9.0 / 64.0 * _1mx2);
	...
	fac = static_cast<btScalar>(9.0 / 64.0 * _1my2);
	...
	fac = static_cast<btScalar>(9.0 / 64.0 * _1mz2);
	...
	if (gradient)
	{
		...
		btScalar _9t3x2py2pz2m19 = static_cast<btScalar>(9.0 * (3.0 * x2 + y2 + z2) - 19.0);
		btScalar _9tx2p3y2pz2m19 = static_cast<btScalar>(9.0 * (x2 + 3.0 * y2 + z2) - 19.0);
		btScalar _9tx2py2p3z2m19 = static_cast<btScalar>(9.0 * (x2 + y2 + 3.0 * z2) - 19.0);
		btScalar _18x = static_cast<btScalar>(18.0 * x);
		btScalar _18y = static_cast<btScalar>(18.0 * y);
		btScalar _18z = static_cast<btScalar>(18.0 * z);

		btScalar _3m9x2 = static_cast<btScalar>(3.0 - 9.0 * x2);
		btScalar _3m9y2 = static_cast<btScalar>(3.0 - 9.0 * y2);
		btScalar _3m9z2 = static_cast<btScalar>(3.0 - 9.0 * z2);

		btScalar _2x = static_cast<btScalar>(2.0 * x);
		btScalar _2y = static_cast<btScalar>(2.0 * y);
		btScalar _2z = static_cast<btScalar>(2.0 * z);
							...
							btVector3* gradient) const
{
	...
	for (unsigned int j = 0u; j < 32u; ++j)
	{
		...
		phi += c * N[j];
		(*gradient)[0] += static_cast<btScalar>(c * dN(j, 0));
		(*gradient)[1] += static_cast<btScalar>(c * dN(j, 1));
		(*gradient)[2] += static_cast<btScalar>(c * dN(j, 2));
	}
	(*gradient) *= c0;
	dist = phi;
	return true;
