...
MEMORY_POOL_DEFINE(RigidBodyComponentOld, 100);

void RigidBodyComponentOld::DebugUI()
{
...
void RigidBodyComponentOld::OnEnable()
{
...
void RigidBodyComponentOld::OnDisable()
{
	auto physicsService = GetOwner().GetWorld().GetService<BPhysicsService>();
...
void RigidBodyComponentOld::SetMass(float mass)
{
	mMass = mass;
...
bool RigidBodyComponentOld::IsDynamic() const
{
	return mMass > 0.0f;
}

btRigidBody* RigidBodyComponentOld::GetRigidBody()
{
	return mRigidBody;
}

void RigidBodyComponentOld::SetConstraints(const Vector3Int& pos, const Vector3Int& rot)
{
...
void RigidBodyComponentOld::AddCollisionMask(CollisionGroups collisionGroup)
{
	if (!mRigidBody)
	{
		return;
...
void RigidBodyComponentOld::RemoveCollisionMask(CollisionGroups collisionGroup)
{
	if (!mRigidBody)
	{
		return;
...
void RigidBodyComponentOld::UpdateConstraint()
{
...
void RigidBodyComponentOld::SetCollisionGroup(CollisionGroups collisionGroup)
{
	if (!mRigidBody)
	{
		return;
