...
void b3GpuGridBroadphase::calculateOverlappingPairs(int maxPairs)
{
	...
	int numSmallAabbs = static_cast<int>(m_smallAabbsMappingGPU.size());
	...
	{
		int numLargeAabbs = static_cast<int>(m_largeAabbsMappingGPU.size());
	...
	if (numSmallAabbs)
	{
...

		{
			...
			if (0)
			{
				...
				int sz = static_cast<int>(m_gpuPairs.size());
...
{
	return static_cast<int>(m_gpuPairs.size());
}
