...
btHeightfieldTerrainShape::getRawHeightFieldValue(int x, int y) const
{
	...
	switch (m_heightDataType)
	{
		...
		case PHY_DOUBLE:
		{
			val = static_cast<btScalar>(m_heightfieldDataDouble[(y * m_heightStickWidth) + x]);
			break;
		}
...
void gridRaycast(Action_T& quadAction, const btVector3& beginPos, const btVector3& endPos, int indices[3])
{
	...
	if (xiStep != 0)
	{
		if (xiStep == 1)
		{
			paramCrossX = static_cast<btScalar>((ceil(beginPos[indices[0]]) - beginPos[indices[0]]) * paramDeltaX);
		}
		else
		{
			paramCrossX = static_cast<btScalar>((beginPos[indices[0]] - floor(beginPos[indices[0]])) * paramDeltaX);
		}
	}
	else
	{
	...
	if (ziStep != 0)
	{
		if (ziStep == 1)
		{
			paramCrossZ = static_cast<btScalar>((ceil(beginPos[indices[2]]) - beginPos[indices[2]]) * paramDeltaZ);
		}
		else
		{
			paramCrossZ = static_cast<btScalar>((beginPos[indices[2]] - floor(beginPos[indices[2]])) * paramDeltaZ);
		}
	}
	else
	{
