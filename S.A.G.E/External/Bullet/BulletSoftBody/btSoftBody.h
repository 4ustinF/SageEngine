...
static const btScalar OVERLAP_REDUCTION_FACTOR = static_cast<btScalar>(0.1);
...
#define SAFE_EPSILON SIMD_EPSILON * 100.0
	void updateNode(btDbvtNode* node, bool use_velocity, bool margin)
	{
		if (node->isleaf())
		{
			...
			vol;
			btScalar pad = margin ? m_sst.radmrg : static_cast<btScalar>(SAFE_EPSILON);  // use user defined margin or margin for floating point precision
			if (use_velocity)
			{
...
#define NEXTRAND (seed = (1664525L * seed + 1013904223L) & 0xffffffff)
		...
		for (int k = 0; k < m_faceNodeContacts.size(); ++k)
		{
			...
			if (vt_norm > SIMD_EPSILON)
			{
				...
				I = static_cast<btScalar>(0.5 * mass * (vt_norm - vt_new));
				vt.safeNormalize();
				I_tilde = static_cast<btScalar>(2.0 * I / (1.0 + w.length2()));
				//                 double the impulse if node or face is constrained.
				if (face_penetration > 0 || node_penetration > 0)
					I_tilde *= 2.0;
				if (face_penetration <= 0)
