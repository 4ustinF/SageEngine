...
static SIMD_FORCE_INLINE btScalar _root3(btScalar x)
{
	...
	btScalar r = 1.5;
	r -= static_cast<btScalar>(1. / 3. * (r - x / (r * r)));
	r -= static_cast<btScalar>(1. / 3. * (r - x / (r * r)));
	r -= static_cast<btScalar>(1. / 3. * (r - x / (r * r)));
	r -= static_cast<btScalar>(1. / 3. * (r - x / (r * r)));
	r -= static_cast<btScalar>(1. / 3. * (r - x / (r * r)));
...
{  // solve equation x^2 + a*x + b = 0
	btScalar D = static_cast<btScalar>(0.25 * a * a - b);
	if (D >= 0)
	{
		D = static_cast<btScalar>(sqrt(D));
		x[0] = static_cast<btScalar>(-0.5 * a + D);
		x[1] = static_cast<btScalar>(-0.5 * a - D);
		return 2;
	}
	x[0] = static_cast<btScalar>(-0.5 * a);
	x[1] = static_cast<btScalar>(sqrt(-D));
	return 0;
}
//---------------------------------------------------------------------------
// x - array of size 3
...
{  // solve cubic equation x^3 + a*x^2 + b*x + c = 0
	...
	if (r2 <= (q3 + eps))
	{  //<<-- FIXED!
		btScalar t = static_cast<btScalar>(r / sqrt(q3));
		if (t < -1)
			t = -1;
		if (t > 1)
			t = 1;
		t = static_cast<btScalar>(acos(t));
		a /= 3;
		q = static_cast<btScalar>(-2 * sqrt(q));
		x[0] = static_cast<btScalar>(q * cos(t / 3) - a);
		x[1] = static_cast<btScalar>(q * cos((t + TwoPi) / 3) - a);
		x[2] = static_cast<btScalar>(q * cos((t - TwoPi) / 3) - a);
		return (3);
	}
	else
	{
		//A =-pow(fabs(r)+sqrt(r2-q3),1./3);
		A = -root3(static_cast<btScalar>(fabs(r) + sqrt(r2 - q3)));
		if (r < 0)
			A = -A;
		B = (A == 0 ? 0 : q / A);
		...
		a /= 3;
		x[0] = (A + B) - a;
		x[1] = static_cast<btScalar>(-0.5 * (A + B) - a);
		x[2] = static_cast<btScalar>(0.5 * sqrt(3.) * (A - B));
		if (fabs(x[2]) < eps)
		{
			x[2] = x[1];
			return (2);
...
{
	btScalar r = static_cast<btScalar>(sqrt(x * x + y * y));
	if (y == 0)
	{
		r = static_cast<btScalar>(sqrt(r));
		if (x >= 0)
		{
			a = r;
			b = 0;
	...
	{  // y != 0
		a = static_cast<btScalar>(sqrt(0.5 * (x + r)));
		b = static_cast<btScalar>(0.5 * y / a);
	}
}
//---------------------------------------------------------------------------
int SolveP4Bi(btScalar* x, btScalar b, btScalar d)  // solve equation x^4 + b*x^2 + d = 0
{
	...
	if (D >= 0)
	{
		btScalar sD = static_cast<btScalar>(sqrt(D));
		btScalar x1 = (-b + sD) / 2;
		btScalar x2 = (-b - sD) / 2;  // x2 <= x1
		if (x2 >= 0)                  // 0 <= x2 <= x1, 4 real roots
		{
			btScalar sx1 = static_cast<btScalar>(sqrt(x1));
			btScalar sx2 = static_cast<btScalar>(sqrt(x2));
			x[0] = -sx1;
			x[1] = sx1;
			x[2] = -sx2;
			x[3] = sx2;
	...
	{  // if( D < 0 ), two pair of compex roots
		btScalar sD2 = static_cast<btScalar>(0.5 * sqrt(-D));
		CSqrt(static_cast<btScalar>(-0.5 * b), sD2, x[0], x[1]);
		CSqrt(static_cast<btScalar>(-0.5 * b), -sD2, x[2], x[3]);
		return 0;
	}  // if( D>=0 )
...
int SolveP4De(btScalar* x, btScalar b, btScalar c, btScalar d)  // solve equation x^4 + b*x^2 + c*x + d
{
	...
	if (res3 > 1)  // 3 real roots,
	{
		...
		{
			btScalar sz1 = static_cast<btScalar>(sqrt(x[0]));
			btScalar sz2 = static_cast<btScalar>(sqrt(x[1]));
			btScalar sz3 = static_cast<btScalar>(sqrt(x[2]));
...
{  // solve equation x^4 + a*x^3 + b*x^2 + c*x + d by Dekart-Euler method
	// move to a=0:
	btScalar d1 = static_cast<btScalar>(d + 0.25 * a * (0.25 * b * a - 3. / 64 * a * a * a - c));
	btScalar c1 = static_cast<btScalar>(c + 0.5 * a * (0.25 * a * a - b));
	btScalar b1 = static_cast<btScalar>(b - 0.375 * a * a);
...
btScalar SolveP5_1(btScalar a, btScalar b, btScalar c, btScalar d, btScalar e)  // return real root of x^5 + a*x^4 + b*x^3 + c*x^2 + d*x + e = 0
{
	...
	btScalar brd = static_cast<btScalar>(fabs(a));  // brd - border of real roots
	if (fabs(b) > brd)
		brd = static_cast<btScalar>(fabs(b));
	if (fabs(c) > brd)
		brd = static_cast<btScalar>(fabs(c));
	if (fabs(d) > brd)
		brd = static_cast<btScalar>(fabs(d));
	if (fabs(e) > brd)
		brd = static_cast<btScalar>(fabs(e));
	...
	if (e < 0)
	{
		...
		f0 = e;
		f1 = F5(x1);
		x2 = static_cast<btScalar>(0.01 * brd);
	}  // positive root
	else
	{
		x0 = -brd;
