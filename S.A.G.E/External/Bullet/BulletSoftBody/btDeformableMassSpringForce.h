	...
	virtual void addScaledDampingForce(btScalar scale, TVStack& force)
	{
		...
		for (int i = 0; i < m_softBodies.size(); ++i)
		{
			...
			for (int j = 0; j < psb->m_links.size(); ++j)
			{
				...
				force[static_cast<int>(id1)] += scaled_force;
				force[static_cast<int>(id2)] -= scaled_force;
			}
		}
	}
	...
	virtual void addScaledDampingForceDifferential(btScalar scale, const TVStack& dv, TVStack& df)
	{
		...
		for (int i = 0; i < m_softBodies.size(); ++i)
		{
			...
			for (int j = 0; j < psb->m_links.size(); ++j)
			{
				...
				btVector3 local_scaled_df = scaled_k_damp * (dv[static_cast<int>(id2)] - dv[static_cast<int>(id1)]);
				if (m_momentum_conserving)
				{
					if ((node2->m_x - node1->m_x).norm() > SIMD_EPSILON)
					{
						btVector3 dir = (node2->m_x - node1->m_x).normalized();
						local_scaled_df = scaled_k_damp * (dv[static_cast<int>(id2)] - dv[static_cast<int>(id1)]).dot(dir) * dir;
					}
				}
				df[static_cast<int>(id1)] += local_scaled_df;
				df[static_cast<int>(id2)] -= local_scaled_df;
			}
		}
	}
			...
			for (int j = 0; j < psb->m_links.size(); ++j)
			{
				...
				if (m_momentum_conserving)
				{
					if ((node2->m_x - node1->m_x).norm() > SIMD_EPSILON)
					{
						...
						{
							if (node1->m_im > 0)
								diagA[static_cast<int>(id1)][d] -= scaled_k_damp * dir[d] * dir[d];
							if (node2->m_im > 0)
								diagA[static_cast<int>(id2)][d] -= scaled_k_damp * dir[d] * dir[d];
						}
					}
				}
				else
				{
					...
					{
						if (node1->m_im > 0)
							diagA[static_cast<int>(id1)][d] -= scaled_k_damp;
						if (node2->m_im > 0)
							diagA[static_cast<int>(id2)][d] -= scaled_k_damp;
					}
				}
			}
		}
	...
	virtual void addScaledElasticForceDifferential(btScalar scale, const TVStack& dx, TVStack& df)
	{
		...
		for (int i = 0; i < m_softBodies.size(); ++i)
		{
			...
			for (int j = 0; j < psb->m_links.size(); ++j)
			{
				...
				btVector3 dx_diff = dx[static_cast<int>(id1)] - dx[static_cast<int>(id2)];
				...
				df[static_cast<int>(id1)] += scaled_df;
				df[static_cast<int>(id2)] -= scaled_df;
			}
		}
	}
