	...
	virtual void addScaledDampingForce(btScalar scale, TVStack& force)
	{
		...
		for (int i = 0; i < m_softBodies.size(); ++i)
		{
			...
			for (int j = 0; j < psb->m_nodes.size(); ++j)
			{
				...
				if (node.m_im > 0)
				{
					force[static_cast<int>(id)] -= scale * node.m_v / node.m_im * m_damping_alpha;
				}
			}
		}
	}
	...
	virtual void addScaledDampingForceDifferential(btScalar scale, const TVStack& dv, TVStack& df)
	{
		...
		for (int i = 0; i < m_softBodies.size(); ++i)
		{
			...
			for (int j = 0; j < psb->m_nodes.size(); ++j)
			{
				...
				if (node.m_im > 0)
				{
					df[static_cast<int>(id)] -= scale * dv[static_cast<int>(id)] / node.m_im * m_damping_alpha;
				}
			}
		}
	}
	...
	virtual void addScaledElasticForceDifferential(btScalar scale, const TVStack& dx, TVStack& df)
	{
		...
		for (int i = 0; i < m_softBodies.size(); ++i)
		{
			...
			for (int j = 0; j < psb->m_tetras.size(); ++j)
			{
				...
				size_t id2 = node2->index;
				size_t id3 = node3->index;
				btMatrix3x3 dF = psb->m_tetraScratches[j].m_corotation.transpose() * Ds(static_cast<int>(id0), static_cast<int>(id1), static_cast<int>(id2), static_cast<int>(id3), dx) * tetra.m_Dm_inverse;
				btMatrix3x3 dP;
