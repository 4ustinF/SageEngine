...
public:
	...
	virtual void testDerivative()
	{
		...
		{
			dx[i].setX(static_cast<btScalar>(randomDouble(-1, 1)));
			dx[i].setY(static_cast<btScalar>(randomDouble(-1, 1)));
			dx[i].setZ(static_cast<btScalar>(randomDouble(-1, 1)));
		}
	...
	virtual void testHessian()
	{
		...
		for (int it = 0; it < 10; ++it)
		{
			...
			error = btSqrt(static_cast<btScalar>(error));
			errors.push_back(error);
