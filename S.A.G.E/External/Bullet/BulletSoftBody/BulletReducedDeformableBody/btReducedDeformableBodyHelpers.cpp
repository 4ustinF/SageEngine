...
btReducedDeformableBody* btReducedDeformableBodyHelpers::createFromVtkFile(btSoftBodyWorldInfo& worldInfo, const char* vtk_file)
{
	...
	while (std::getline(fs, line))
	{
		...
		else if (line.substr(0, 6) == "POINTS")
		{
			...
			ss >> n_points;
			X.resize(static_cast<int>(n_points));
		}
		else if (line.substr(0, 5) == "CELLS")
		{
			...
			ss >> n_tets;
			indices.resize(static_cast<int>(n_tets));
		}
		...
		else if (reading_points)
		{
			...
			X[static_cast<int>(x_count++)] = position;
		}
		else if (reading_tets)
		{
			...
			//printf("\n");
			indices[static_cast<int>(indices_count++)] = tet;
		}
	}
	btReducedDeformableBody* rsb = new btReducedDeformableBody(&worldInfo, static_cast<int>(n_points), &X[0], 0);
