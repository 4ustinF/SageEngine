...
public:
	...
	void updateYoungsModulusAndPoissonRatio()
	{
		...
		m_nu = static_cast<btScalar>(m_lambda * 0.5 / (m_mu + m_lambda));
	}
	...
	void updateLameParameters()
	{
		...
		// https://en.wikipedia.org/wiki/Lam%C3%A9_parameters
		m_mu = static_cast<btScalar>(m_E * 0.5 / (1 + m_nu));
	...
	virtual void addScaledDampingForce(btScalar scale, TVStack& force)
	{
		...
		for (int i = 0; i < m_softBodies.size(); ++i)
		{
			...
			for (int j = 0; j < psb->m_tetras.size(); ++j)
			{
				...
				force[static_cast<int>(id0)] -= scale1 * df_on_node0;
				force[static_cast<int>(id1)] -= scale1 * df_on_node123.getColumn(0);
				force[static_cast<int>(id2)] -= scale1 * df_on_node123.getColumn(1);
				force[static_cast<int>(id3)] -= scale1 * df_on_node123.getColumn(2);
			}
		}
	}
	...
	virtual void addScaledElasticForce(btScalar scale, TVStack& force)
	{
		...
		for (int i = 0; i < m_softBodies.size(); ++i)
		{
			...
			for (int j = 0; j < psb->m_tetras.size(); ++j)
			{
				...
				// elastic force
				btScalar scale1 = scale * tetra.m_element_measure;
				force[static_cast<int>(id0)] -= scale1 * force_on_node0;
				force[static_cast<int>(id1)] -= scale1 * force_on_node123.getColumn(0);
				force[static_cast<int>(id2)] -= scale1 * force_on_node123.getColumn(1);
				force[static_cast<int>(id3)] -= scale1 * force_on_node123.getColumn(2);
			}
		}
	}
	...
	virtual void addScaledDampingForceDifferential(btScalar scale, const TVStack& dv, TVStack& df)
	{
		...
		for (int i = 0; i < m_softBodies.size(); ++i)
		{
			...
			for (int j = 0; j < psb->m_tetras.size(); ++j)
			{
				...
				size_t id2 = node2->index;
				size_t id3 = node3->index;
				btMatrix3x3 dF = Ds(static_cast<int>(id0), static_cast<int>(id1), static_cast<int>(id2), static_cast<int>(id3), dv) * tetra.m_Dm_inverse;
				btMatrix3x3 I;
				I.setIdentity();
				btMatrix3x3 dP = (dF + dF.transpose()) * m_mu_damp + I * (dF[0][0] + dF[1][1] + dF[2][2]) * m_lambda_damp;
				//                firstPiolaDampingDifferential(psb->m_tetraScratchesTn[j], dF, dP);
				...
				// damping force differential
				btScalar scale1 = scale * tetra.m_element_measure;
				df[static_cast<int>(id0)] -= scale1 * df_on_node0;
				df[static_cast<int>(id1)] -= scale1 * df_on_node123.getColumn(0);
				df[static_cast<int>(id2)] -= scale1 * df_on_node123.getColumn(1);
				df[static_cast<int>(id3)] -= scale1 * df_on_node123.getColumn(2);
			}
		}
	}
	...
	virtual void addScaledElasticForceDifferential(btScalar scale, const TVStack& dx, TVStack& df)
	{
		...
		for (int i = 0; i < m_softBodies.size(); ++i)
		{
			...
			for (int j = 0; j < psb->m_tetras.size(); ++j)
			{
				...
				size_t id2 = node2->index;
				size_t id3 = node3->index;
				btMatrix3x3 dF = Ds(static_cast<int>(id0), static_cast<int>(id1), static_cast<int>(id2), static_cast<int>(id3), dx) * tetra.m_Dm_inverse;
				btMatrix3x3 dP;
	...
	{
		btScalar c1 = static_cast<btScalar>((m_mu * (1. - 1. / (s.m_trace + 1.))));
		btScalar c2 = static_cast<btScalar>((m_lambda * (s.m_J - 1.) - 0.75 * m_mu));
	...
	{
		btScalar c1 = static_cast<btScalar>(m_mu * (1. - 1. / (s.m_trace + 1.)));
		btScalar c2 = static_cast<btScalar>((2. * m_mu) * DotProduct(s.m_F, dF) * (1. / ((1. + s.m_trace) * (1. + s.m_trace))));
		btScalar c3 = (m_lambda * DotProduct(s.m_cofF, dF));
		dP = dF * c1 + s.m_F * c2;
		addScaledCofactorMatrixDifferential(s.m_F, dF, static_cast<btScalar>(m_lambda * (s.m_J - 1.) - 0.75 * m_mu), dP);
		dP += s.m_cofF * c3;
	}
	...
	{
		btScalar c1 = static_cast<btScalar>((m_mu_damp * (1. - 1. / (s.m_trace + 1.))));
		btScalar c2 = static_cast<btScalar>(((2. * m_mu_damp) * DotProduct(s.m_F, dF) * (1. / ((1. + s.m_trace) * (1. + s.m_trace)))));
		btScalar c3 = (m_lambda_damp * DotProduct(s.m_cofF, dF));
		dP = dF * c1 + s.m_F * c2;
		addScaledCofactorMatrixDifferential(s.m_F, dF, static_cast<btScalar>(m_lambda_damp * (s.m_J - 1.) - 0.75 * m_mu_damp), dP);
		dP += s.m_cofF * c3;
	}
