...
void btDeformableBackwardEulerObjective::multiply(const TVStack& x, TVStack& b) const
{
	...
	for (int i = 0; i < m_softBodies.size(); ++i)
	{
		...
		for (int j = 0; j < psb->m_nodes.size(); ++j)
		{
			...
			b[static_cast<int>(counter)] = (node.m_im == 0) ? btVector3(0, 0, 0) : x[static_cast<int>(counter)] / node.m_im;
			++counter;
		}
	}
	...
	for (int i = 0; i < m_softBodies.size(); ++i)
	{
		...
		if (m_implicit)
		{
			for (int j = 0; j < psb->m_nodes.size(); ++j)
			{
				if (psb->m_nodes[j].m_im != 0)
				{
					psb->m_nodes[j].m_v += psb->m_nodes[j].m_effectiveMass_inv * force[static_cast<int>(counter++)];
				}
			}
		}
		else
		{
			for (int j = 0; j < psb->m_nodes.size(); ++j)
			{
				...
				psb->m_nodes[j].m_v += one_over_mass * force[static_cast<int>(counter++)];
			}
		}
	}
	if (setZero)
...
void btDeformableBackwardEulerObjective::initialGuess(TVStack& dv, const TVStack& residual)
{
	...
	for (int i = 0; i < m_softBodies.size(); ++i)
	{
		...
		for (int j = 0; j < psb->m_nodes.size(); ++j)
		{
			dv[static_cast<int>(counter)] = psb->m_nodes[j].m_im * residual[static_cast<int>(counter)];
			++counter;
		}
	}
}
