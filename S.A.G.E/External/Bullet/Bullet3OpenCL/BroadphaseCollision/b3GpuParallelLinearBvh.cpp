								   ...
								   const b3OpenCLArray<int>& largeAabbIndices)
{
	...
	int numLargeAabbs = static_cast<int>(largeAabbIndices.size());
	int numSmallAabbs = static_cast<int>(smallAabbIndices.size());
...
{
	int maxPairs = static_cast<int>(out_overlappingPairs.size());
	...
	if (m_leafNodeAabbs.size() > 1)
	{
		...
		int numQueryAabbs = static_cast<int>(m_leafNodeAabbs.size());
	...
	int numLargeAabbRigids = static_cast<int>(m_largeAabbs.size());
													 ...
													 b3OpenCLArray<int>& out_numRayRigidPairs, b3OpenCLArray<b3Int2>& out_rayRigidPairs)
{
	...
	int numRays = static_cast<int>(rays.size());
	int maxRayRigidPairs = static_cast<int>(out_rayRigidPairs.size());
...
void b3GpuParallelLinearBvh::constructBinaryRadixTree()
{
	...
	int numLeaves = static_cast<int>(m_leafNodeAabbs.size());
	int numInternalNodes = numLeaves - 1;
