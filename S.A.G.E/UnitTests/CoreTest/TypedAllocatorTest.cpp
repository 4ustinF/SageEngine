...
namespace CoreTest
{
	...
	public:
		...
		{
			TypedAllocator<Foo> typedAllocator(1);
			Foo* ptr = typedAllocator.New(36, 48); // Overloaded constructor on Foo(int, int)
			Assert::IsNotNull(ptr);
		...
		TEST_METHOD(TestForwardNew)
		{
			TypedAllocator<Foo> typedAllocator(1);
			Foo* ptr = typedAllocator.New(Bar()); // Overloaded constructor on Foo(Bar), bar should be moved
			Assert::IsNotNull(ptr);
			Assert::IsTrue(ptr->a == 12);
			Assert::IsTrue(ptr->b == 34);
		...
		TEST_METHOD(TestRecycling)
		{
			...
			typedAllocator.Delete(ptr1); // destructor called
